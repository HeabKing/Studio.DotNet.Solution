...
function flashPercentage() {
	// img 标签数组
	window.imgTags = document.getElementsByTagName("img");
	// 当前下载完成的图片的百分比, 默认为没有图片, 进度条100%
	window.ImgCompletePercetage = 1;
	// 不管图片下载成功还是失败, 只要不是正在下载, 那么就算他完成了
	for (var index = 0; index < window.imgTags.length; index++) {
		if (window.imgTags[index].complete === true) {
			window.ImgCompletePercetage = (index + 1) / window.imgTags.length;
		}
	}

	// 如果进度条不存在并且所有图片都加载完成了
	if (NProgress.status == null && window.ImgCompletePercetage === 1) {
		return;
	}
	// 因为进度条在一直下载的时候能够每一秒添加1%作为效果, 所以要设置以下代码
	if (NProgress.status != null && window.ImgCompletePercetage <= NProgress.status) {
		return;
	}
	NProgress.set(window.ImgCompletePercetage);
}
// 设置轮询 - 不关闭的原因是如果异步加载了一些<img>那么能够立刻再次显示进度条
$(function () {
